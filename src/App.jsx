      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '80vh', backgroundColor: '#f9f9f9' }}>
        <div style={{ ...styles.card, width: '400px' }}>
          ...
          <form onSubmit={(e) => {
            ...
            <label style={{ fontWeight: 'bold', fontSize: '0.9em' }}>Nombre de Usuario</label>
            ...
            <button type="submit" style={{ ...styles.btnMain, width: '100%', marginTop: '15px' }}>Iniciar Sesión</button>
          </form>
        </div>
      </div>
    );
    ...
    return (
      <div style={styles.container}>
        <div style={styles.nav}>
          ...
          <button onClick={handleLogout} style={{ background: 'none', color: '#ff4d4d', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}>Cerrar Sesión</button>
        </div>
        <div style={{ ...styles.card, textAlign: 'center' }}>
          <h1 style={{ fontSize: '2.5rem', color: '#333' }}>¡Bienvenido!</h1>
          <p style={{ color: '#666', marginBottom: '30px' }}>Gestiona los insumos y el stock de tu puesto de hamburguesas de forma sencilla.</p>
          <button 
            onClick={() => setView('inventory')} 
            style={{ ...styles.btnMain, width: 'auto', padding: '15px 40px' }}
          >
...
const InventoryView = ({ onBack, styles, onLogout }) => {
  ...
  const fetchIngredientes = () => {
    fetch('http://localhost:8081/ingredientes')
      .then(res => res.json())
  ...
  const handleCrear = (e) => {
    e.preventDefault();
    fetch('http://localhost:8081/crear', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleEliminar = (id) => {
    if (window.confirm("¿Deseas eliminar este ingrediente de la lista?")) {
      fetch(`http://localhost:8081/eliminar/${id}`, { method: 'DELETE' })
  ...
  const handleEditar = (id, actual) => {
    ...
    if (nuevaCant !== null) {
      fetch(`http://localhost:8081/actualizar/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
